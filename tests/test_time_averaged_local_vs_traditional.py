...
def test_time_averaged_local_measures_match_traditional_on_disk_data():
    ...
    _, wrapper_res = time_averaged_local_measures(
        X_sub,
        min_order=N,
        max_order=N,
        device="cpu",
    ...
    # wrapper_res is the averaged measures dict; key N gives [n_combinations, D, 4]
    meas = wrapper_res[N].squeeze(0).squeeze(0).cpu().numpy()
