...
def test_local_vs_traditional_measures():
    ...
    _, local_averaged_results = time_averaged_local_measures(
        data, min_order=3, max_order=4,
        device='cpu', dtype=torch.float64,
...
def test_bias_correction_application():
    ...
    _, averaged_no_bias = time_averaged_local_measures(
        data, min_order=3, max_order=3, device='cpu', dtype=torch.float64,
        bias_correction=False
    )

    # Get time-averaged measures WITH bias correction
    _, averaged_with_bias = time_averaged_local_measures(
        data, min_order=3, max_order=3, device='cpu', dtype=torch.float64,
        bias_correction=True
    )
...
def test_performance_comparison():
    ...
    _, covmats = gaussian_copula_covmat_batched(data)
    ...
    start = time.time()
    _, local_results = time_averaged_local_measures(
        data, covmats=covmats, min_order=3, max_order=4,
