...
def _get_bias_correctors(T: Optional[List[int]], orders: torch.Tensor, batch_size: int, N: int, D: int, device: torch.device):
    ...
    if T is not None:
        ...
        bias_correctors = torch.tensor([
            [_gaussian_entropy_bias_correction(order,t) for order in range(1,N+1)]
            for t in T
        ], device=device)
    ...
    # |batch_size x D|
    bc1 = bias_correctors[0].repeat(D)
...
def _get_tc_dtc_from_batched_covmat(covmats: torch.Tensor, 
    ...
    # |batch_size| x |N|
    var_ents = _gaussian_entropy_estimation(single_var_covmats, 1) - bc1
    var_ents = var_ents * nplets # Set as zero the elements to ignore (nplets = 0)
    # |batch_size| x |N|
    single_exclusion_ents = _gaussian_entropy_estimation(single_exclusion_covmats, N-1) - bcOrdermin1[:,None]
